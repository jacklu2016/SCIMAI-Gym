...
bar = rects1[0]
# 在柱子上方标注
ax.text(bar.get_x() + bar.get_width() / 2,bar.get_height() + 100,f"Min: {values_20[0]:.2f}",ha="center", va="bottom", fontsize=8, color="black")
rects2 = ax.bar(x - width/2, values_50, width, label='50家药店', color=academic_palette[1])
bar = rects2[0]
# 在柱子上方标注
ax.text(bar.get_x() + bar.get_width() / 2,bar.get_height() + 100,f"Min: {values_50[0]:.2f}",ha="center", va="bottom", fontsize=8, color="black")

rects3 = ax.bar(x + width/2, values_80, width, label='80家药店', color=academic_palette[2])
bar = rects3[0]
# 在柱子上方标注
ax.text(bar.get_x() + bar.get_width() / 2,bar.get_height() + 100,f"Min: {values_80[0]:.2f}",ha="center", va="bottom", fontsize=8, color="black")

rects4 = ax.bar(x + 3*width/2, values_100, width, label='100家药店', color=academic_palette[3])
bar = rects4[3]
# 在柱子上方标注
ax.text(bar.get_x() + bar.get_width() / 2,bar.get_height() + 100,f"Min: {values_100[3]:.2f}",ha="center", va="bottom", fontsize=8, color="black")
...
plt.tight_layout()
#plt.show()
plt.savefig('inv.svg',format='svg',bbox_inches='tight')