...
def prob_linear_rank(rm, w1, w2, xln_start, M, current_sol):
    ...
    n_neg = 2 / (rm + 1)
    n_pos = (2 * rm) / (rm + 1)
    p2_prob = {p2_dict[i][0]: (1 / M) * (n_neg + (n_pos - n_neg) * (((M - i) - 1) / (M - 1))) for i in
               range(len(p2_dict))}
...
def mating(parent_1, parent_2, num_stores, num_parts, method='Single'):
    ...
    if method == 'Single':
        ...
        print(offspring_4)
        offspring = [pd.DataFrame(offspring_1), pd.DataFrame(offspring_2), pd.DataFrame(offspring_3),
                     pd.DataFrame(offspring_4)]
    return offspring
